...
def findDirectory(path, re, token):
  ...
  for dirn in alldirs:
    ...
    if not os.path.isfile(dirn + '/' + tmpFileName): continue
    # if token did not produce any stats file (e.g. not applicable to dns):
    if returndir is None: returndir = dirn
    else: assert False, 'Invalid token %s, found in too many dirs' % token
  assert returndir is not None, "token %s - %d not found" % (token, re)
  return returndir
...
if __name__ == "__main__":
    ...
    parser.add_argument('simList', nargs='+', help="Path of simulations to process.")
    ...
    parser.add_argument('-r', '--runspath',  nargs='+', default=['./data/'],
      help="Plot labels to assiciate to tokens")

    args = parser.parse_args()
    if len(args.runspath) == 1: args.runspath = args.runspath * len(args.simList)
    ...
    for i, sim_t in enumerate(args.simList):
      for j, re in enumerate(args.Re):
        if  0: sim_dir = sim_t + '_RE%03d/' % re
        else : sim_dir = findDirectory(args.runspath[i], re, sim_t)
