...
from plot_histograms_CSS      import readSGSfile
...
def getSGSdissip(dirname, dissip_visc, nu, i, j):
    dissip_SGS_coef, _, _, _ = readSGSfile(dirname)
    dissip_SGS = dissip_visc * (dissip_SGS_coef / nu)
    return dissip_SGS
...
def main_integral(runspath, target, tokens):
    ...
    for i, token in enumerate(tokens):
        for j, RE in enumerate(REs):
            ...
            dirn = findBestHyperParams(runspath[i], RE, token, logSpectra, logEnStdev)
